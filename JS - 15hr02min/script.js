...
function onClickListItem () {
    ...
    total.innerHTML = selectedPhotos.length;
}
for(index = 0; index < photos.length; index++) {
		photos[index].addEventListener('click', onClickListItem);
	}
...
var texte = document.createTextNode("Item 4");
...
var nouveau = document.createTextNode("Paragraphe 3");
var ancien = p.childNodes[0];
p.replaceChild(nouveau, ancien);
*/

// --------------------------------------------------------------------------------------------------
                                        //		Fin Bravo !! Total count : 15h02min